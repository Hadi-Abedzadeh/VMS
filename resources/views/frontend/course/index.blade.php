...
@section('content')
    <main class="main-responsive">
        <div class="title-page">
            <div class="container">
                <div class="row">
                    <h1>{{ trans('ui.text.courses') }}</h1>
                    <p>{{ trans('ui.text.courses_text') }}</p>
                </div>
            </div>
        </div>
                    ...
                    @foreach($courses as $post)
                        <div class="col-xl-4 col-lg-6 col-md-6 col-sm-12 col-xs-12">
                            <div class="blog-item">
                                ...
                                <a href="{{ route('courses.list', $post->slug) }}">{{$post->title}}</a>
                                <div class="footer-item">
                                    <div class="col-xl-4 col-xs-6">
                                        <a href="{{ route('courses.list', $post->slug)}}">{{ trans('ui.text.enter_course') }}</a></div>
                                </div>
                            </div>
                        </div>
                    @endforeach
