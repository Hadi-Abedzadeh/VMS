                ...
                <div class="row">
                    ...
                    @foreach($categories as $category)
                        @foreach($category->posts as $posts)
                            ...
                            <div class="blog-item">
                                <div class="thumbnail" style="background-image: url('{{ $posts->image }}')"></div>
                                <a href="{{ route('blog.show', $posts->slug) }}">{{$posts->title}}</a>
                                <div class="footer-item">
                                    <div class="col-xl-4 col-xs-6">
                                        <a href="{{ route('blog.show', $posts->slug)}}">{{ trans('ui.text.read_more') }}</a></div>
                                </div>
                            </div>
                        </div>
                    @endforeach
